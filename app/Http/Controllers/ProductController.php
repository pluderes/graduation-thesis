...
class ProductController extends Controller
{
    ...
    public function details_product($prod_id)
    {
        ...
        $detais_product = DB::table('product')->join('product_status', 'product.status_id', '=', 'product_status.status_id')->join('type', 'product.type_id', '=', 'type.type_id')->join('category', 'type.cate_id', '=', 'category.cate_id')->where('product.prod_id', $prod_id)->get();
        ...
        $prod_by_status = DB::table('product')->join('product_status', 'product.status_id', '=', 'product_status.status_id')->where('product.status_id', '=', '3')->paginate(3);
        $prod_by_cate = DB::table('product')->join('product_status', 'product.status_id', '=', 'product_status.status_id')->join('type', 'product.type_id', '=', 'type.type_id')->join('category', 'type.cate_id', '=', 'category.cate_id')->where('category.cate_id', $cate_id)->whereNotIn('product.prod_id', [$prod_id])->limit(4)->get();

        // echo '<pre>';
        // printf($detais_product);
        // echo '</pre>';
